...
int main() {
    ...
    for(int i=0;i<10;i++){
        int k=rand()%5;
        pthread_create(&viagg[i],&attr,Viaggiatori,(void *)m[k]);
    }    
    
    /* TODO: Effettuare la join con i thread "gestoreMolo" */
    int STATUSG;
    for(int i=0;i<5;i++){
        pthread_join(gestore_molo[i],(void**)&STATUSG);
        printf("Gestore %d terminato con stato %d\n",i,STATUSG);
    ...
    int STATUSV;
     ...
     for(int i=0;i<10;i++){
        pthread_join(viagg[i],(void**)&STATUSV);
        printf("Viaggiatore %d terminato con stato %d\n",i,STATUSV);
