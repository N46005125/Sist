...
int main()
{
    ...
    vett_id = shmget(IPC_PRIVATE, sizeof(int[NUM_ELEMENTI]), IPC_CREAT | IPC_EXCL | 0664 );/* TBD: usare shmget() per creare un vettore di interi su 
    ...
    buffer_id = shmget(IPC_PRIVATE, sizeof(int), IPC_CREAT | IPC_EXCL | 0664 );/* TBD: usare shmget() per creare un buffer singolo su
