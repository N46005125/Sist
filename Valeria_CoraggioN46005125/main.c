...
int main(){
        ...
        //create queues
        int key_queue_gen_filter = IPC_PRIVATE; /* TODO: definire chiave coda generatore-filtro */
        int key_queue_filter_checksum = IPC_PRIVATE; /* TODO: definire chiave coda filtro-checksum */
        int key_queue_checksum_visual = IPC_PRIVATE; /* TODO: definire chiave coda checksum-visualizzatore */

        ds_queue_gen_filter = msgget(key_queue_gen_filter,IPC_CREAT|0664);  /* TODO: generare il descrittore coda generatore-filtro */
        if(ds_queue_gen_filter<0) { perror("queue error"); exit(1); }
        ...
        if(ds_queue_filter_checksum<0) { perror("queue error"); exit(1); }
        ...
        if(ds_queue_checksum_visual<0) { perror("queue error"); exit(1); }
        ...
        key_t shm_key = IPC_PRIVATE;/* TODO: definire chiave shared memory per prod/cons generatori*/
        ...
        struct ProdConsGen * pc = (struct ProdConsGen *) shmat(ds_shm,0,0);
